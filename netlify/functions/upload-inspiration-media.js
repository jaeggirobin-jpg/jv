// upload-inspiration-media.js — Bild oder Video für einen Inspirations-Punkt hochladen (Admin)
...
// Erlaubte MIME-Types und Maximalgrößen
...
exports.handler = async (event) => {
  ...
  const { punkt_id, file_base64, file_name, media_type } = payload;

  // Validierung
  if (!punkt_id) {
    return respond(400, { error: 'punkt_id fehlt' });
  }
  if (!file_base64) {
  ...
  const { data: punkt, error: punktError } = await supabase
    ...
    .select('id, kategorie, bild_url, video_url')
    .eq('id', punkt_id)
    .maybeSingle();
  ...
  const storagePath = `${punkt.kategorie}/${punkt_id}_${safeName}`;
